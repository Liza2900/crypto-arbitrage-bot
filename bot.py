...
if not BOT_TOKEN or not WEBHOOK_URL:
    raise ValueError("❌ BOT_TOKEN або WEBHOOK_URL не встановлені в середовищі.")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Натисніть кнопку нижче, щоб знайти арбітраж 👇",
    ...
    if context.user_data.get("awaiting_min_profit_usd"):
        try:
            ...
            await context.bot.send_message(
                chat_id,
                f"✅ Мінімальний прибуток встановлено: {min_profit} $",
                reply_markup=settings_keyboard(filters)
            )
        except:
